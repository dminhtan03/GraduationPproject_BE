...
public interface TaskRepository extends JpaRepository<Task, String> {
    ...
    /** All tasks in a project visible to any project member.
     *  Members of the project (via tbl_project_member) can see ALL tasks.
     *  Non-members only see tasks they created/review/are assigned to/support. */
    @Query("SELECT DISTINCT t FROM Task t " +
           "LEFT JOIN TaskAssignment a ON a.task = t " +
           "LEFT JOIN TaskSupporter s ON s.task = t " +
           "WHERE t.project.id = :projectId " +
           "AND t.parentTask IS NULL " +
           "AND (:search IS NULL OR LOWER(t.title) LIKE LOWER(CONCAT('%', :search, '%'))) " +
           "AND (:status IS NULL OR t.status = :status) " +
           "AND (EXISTS (SELECT pm FROM ProjectMember pm WHERE pm.project.id = :projectId AND pm.user.id = :userId) " +
           "     OR t.createdBy.id = :userId OR t.reviewer.id = :userId " +
           "     OR a.assignee.id = :userId OR s.user.id = :userId) " +
           "ORDER BY t.createdAt DESC")
