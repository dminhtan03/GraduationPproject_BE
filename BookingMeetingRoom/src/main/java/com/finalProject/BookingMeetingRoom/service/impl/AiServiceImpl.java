...
public class AiServiceImpl implements AiService {
    ...
    public AiChatResponse chat(AiChatRequest request, Authentication authentication) {
        try {
            ...
            String message = originalMessage != null ? originalMessage.toLowerCase(Locale.ROOT) : "";

            User user = (authentication != null)
                ? userRepository.findByEmail(authentication.getName()).orElse(null)
                : null;

            chatHistoryService.log(user, sessionId, SenderType.USER, originalMessage);
            ...
            if (containsAny(message, "đặt phòng", "dat phong", "book", "reserve", "đặt lịch", "dat lich")) {
                return handleBookingIntent(request, authentication, sessionId, user);
            }
            ...
            if (containsAny(message, "gợi ý", "goi y", "phòng trống", "phong trong", "suggest")) {
                return handleSuggestionIntent(request, sessionId, user);
            }
            ...
            AiChatResponse res = AiChatResponse.builder()
                    .sessionId(sessionId)
                    .reply(reply)
                    .reservationCreated(false)
                    .build();
            chatHistoryService.log(user, sessionId, SenderType.BOT, reply);
            return res;
    ...
    public AiChatResponse reserveViaAi(ReservationRequest request, Authentication authentication) {
        ...
        private AiChatResponse handleBookingIntent(AiChatRequest request, Authentication authentication, String sessionId, User user) {
        ...
        if (candidates.isEmpty()) {
            String reply = "Hiện tại không tìm thấy phòng nào đủ sức chứa cho " + request.getCapacity() + " người.";
            AiChatResponse res = AiChatResponse.builder()
                .sessionId(sessionId)
        ...
        for (Room room : candidates) {
            ...
            String reply = "Xin lỗi, không còn phòng trống phù hợp trong khung giờ bạn yêu cầu.";
            AiChatResponse res = AiChatResponse.builder()
                .sessionId(sessionId)
                .reply(reply)
                .reservationCreated(false)
            ...
            private AiChatResponse handleSuggestionIntent(AiChatRequest request, String sessionId, User user) {
        ...
        if (suggestions.isEmpty()) {
            String reply = "Không tìm thấy phòng trống phù hợp trong thời gian bạn yêu cầu.";
            AiChatResponse res = AiChatResponse.builder()
                .sessionId(sessionId)
                .reply(reply)
                .reservationCreated(false)
